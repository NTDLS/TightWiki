...
INSERT INTO ConfigurationEntry(ConfigurationGroupId, Name, Value, DataTypeId, Description, IsEncrypted, IsRequired)
SELECT (SELECT Id FROM ConfigurationGroup WHERE Name = 'LDAP Authentication' LIMIT 1), 'LDAP : Enable LDAP Authentication', '0', 3, 'Whether or not to authenticate with LDAP when attempting to login users.', 0, 1
...
INSERT INTO ConfigurationEntry(ConfigurationGroupId, Name, Value, DataTypeId, Description, IsEncrypted, IsRequired)
SELECT (SELECT Id FROM ConfigurationGroup WHERE Name = 'LDAP Authentication' LIMIT 1), 'LDAP : Fully-Qualified Domain', '', 2, 'The fully qualified domain name to use when performing LDAP authentication.', 0, 0
...
INSERT INTO ConfigurationEntry(ConfigurationGroupId, Name, Value, DataTypeId, Description, IsEncrypted, IsRequired)
SELECT (SELECT Id FROM ConfigurationGroup WHERE Name = 'LDAP Authentication' LIMIT 1), 'LDAP : Use Secure Socket Layer', '0', 3, 'Whether or not to use a secure connection to LDAP. This is not always supported by domains.', 0, 1
...
INSERT INTO ConfigurationEntry(ConfigurationGroupId, Name, Value, DataTypeId, Description, IsEncrypted, IsRequired)
SELECT (SELECT Id FROM ConfigurationGroup WHERE Name = 'LDAP Authentication' LIMIT 1), 'LDAP : Default Sign-in Domain', '', 2, 'The default domain to use when the user does not specify a UPN or a fully qualified domain\\Username when signing in.', 0, 0
ON CONFLICT(ConfigurationGroupId, Name) DO UPDATE SET Name = 'LDAP : Default Sign-in Domain', DataTypeId = 2, Description = 'The default domain to use when the user does not specify a UPN or a fully qualified domain\\Username when signing in.', IsEncrypted = '0', IsRequired = '0';
