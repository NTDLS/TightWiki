...
namespace TightWiki.Controllers
{
        ...
        : TwController<PageController>(logger, signInManager, userManager, localizer, wikiConfiguration, databaseManager)
    {
        ...
        private async Task TryIncrementPageView(int pageId)
        {
            try
            {
                var hitCounterThrottle = await configurationRepository.Get(TwConfigGroup.Performance, "Hit Counter Throttle (Seconds)", 0);
                var cacheKey = MemCacheKey.Build(MemCache.Category.Session, [pageId, "PageView", GetPageViewViewerKey()]);
                ...
                MemCache.Set(cacheKey, DateTime.UtcNow, TimeSpan.FromSeconds(hitCounterThrottle));
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult> Display(string givenCanonical, int? pageRevision)
        {
            ...
            try
            {
                    ...
                    if (WikiConfiguration.PageCacheSeconds > 0)
                    {
                        ...
                        else
                        {
                            ...
                            if (state.ProcessingInstructions.Contains(TwInstruction.NoCache) == false)
                            {
                                ...
                                MemCache.Set(cacheKey, toBeCached); //This is cleared with the call to Cache.ClearCategory($"Page:{page.Navigation}");
                            }
                        }
                    }
                    else
                ...
                {
                    var notExistPageName = await configurationRepository.Get<string>(TwConfigGroup.Customization, "Revision Does Not Exists Page");
                ...
                {
                    var notExistPageName = await configurationRepository.Get<string>(TwConfigGroup.Customization, "Page Not Exists Page");
                    string notExistPageNavigation = TwNamespaceNavigation.CleanAndValidate(notExistPageName);
                    var notExistsPage = (await pageRepository.GetPageRevisionByNavigation(notExistPageNavigation)).EnsureNotNull();
        ...
        public async Task<ActionResult> Delete(string givenCanonical, PageDeleteViewModel model)
        {
            try
            {
                ...
                if (instructions.Contains(TwInstruction.Protect))
                {
        ...
        public async Task<ActionResult> Edit(string givenCanonical)
        {
            try
            {
                ...
                else
                {
                    ...
                    string templateName = (await configurationRepository.Get<string>(TwConfigGroup.Customization, "New Page Template")).EnsureNotNull();
        ...
        public async Task<ActionResult> Edit(PageEditViewModel model)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        MemCache.ClearCategory(MemCacheKey.Build(MemCache.Category.Page, [originalNavigation]));
                        MemCache.ClearCategory(MemCacheKey.Build(MemCache.Category.Page, [page.Id]));
        ...
        public async Task<ActionResult> Image(string givenPageNavigation, string givenFileNavigation, int? pageRevision = null)
        {
            try
            {
                ...
                var cacheKey = MemCacheKeyFunction.Build(MemCache.Category.Page, [givenPageNavigation, givenFileNavigation, pageRevision, scale, maxWidth]);
